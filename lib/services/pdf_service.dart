...
class PdfService {
  ...
  static Future<String> generateReport({
    ...
    // Build metrics
    final metrics = _buildMetrics(result);
          ...
          ...suggestions.map(
            (s) => pw.Padding(
              ...
              child: pw.Row(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('• ', style: pw.TextStyle(fontSize: 12, color: PdfColors.green700)),
                  pw.Expanded(
                    child: pw.Text(
                      s,
                      style: pw.TextStyle(fontSize: 11, color: PdfColors.blueGrey700),
  ...
  static pw.Widget _buildMetricRow(_PdfMetric m) {
    final isGood = m.score >= 0.7;
    final barColor = isGood ? PdfColors.green600 : PdfColors.red600;
    final ratingColor = isGood ? PdfColors.green700 : PdfColors.red700;
    final pct = (m.score * 100).clamp(0, 100).toStringAsFixed(0);
        ...
        children: [
          ...
          pw.Stack(
            children: [
              ...
              pw.Container(
                height: 6,
                width: 500 * m.score.clamp(0.0, 1.0),
                decoration: pw.BoxDecoration(
                  color: barColor,
                  borderRadius: pw.BorderRadius.circular(3),
                ),
  ...
  static List<_PdfMetric> _buildMetrics(PredictionResult result) {
    final base = result.confidence;
    final isDyslexic = result.label == 'Dyslexic';
    if (isDyslexic) {
      return [
        _PdfMetric('Letter Recognition', 1 - base * 0.6, 'Poor'),
        _PdfMetric('Spacing Consistency', 1 - base * 0.5, 'Fair'),
        _PdfMetric('Baseline Alignment', 1 - base * 0.7, 'Poor'),
        _PdfMetric('Letter Formation', 1 - base * 0.55, 'Fair'),
        _PdfMetric('Pressure Consistency', 1 - base * 0.4, 'Fair'),
      ];
    } else {
      return [
        _PdfMetric('Letter Recognition', base, 'Excellent'),
        _PdfMetric('Spacing Consistency', base * 0.98, 'Excellent'),
        _PdfMetric('Baseline Alignment', base, 'Excellent'),
        _PdfMetric('Letter Formation', base, 'Excellent'),
        _PdfMetric('Pressure Consistency', base * 0.99, 'Excellent'),
      ];
    }
  }
...
class _PdfMetric {
  ...
  final double score;
  final String rating;
  const _PdfMetric(this.label, this.score, this.rating);
}
